...
void fastjetppgenkt_pty_(const double * p, const int & npart,                   
    ...
    fj::Selector select_rap = fj::SelectorAbsRapMax(maxrap);
    fj::Selector select_pt = fj::SelectorPtMin(ptmin);
    fj::Selector select_both = select_rap && select_pt;
    vector<fj::PseudoJet> jets = select_both(cs.inclusive_jets());
